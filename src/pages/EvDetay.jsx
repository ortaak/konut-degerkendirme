...
import { ArrowLeft, Star, Info, Check, MessageSquare, ChevronDown, ChevronUp, BarChart2 } from 'lucide-react'
...
export default function EvDetay() {
  ...
  const { evler, kriterler, evGuncelle, puanKaydet } = useApp()
  ...
  const { yuzde, tamamlananKriter } = toplamPuanHesapla(aktifKriterler, ev.puanlar || {})
  const sinif = skorSinifi(yuzde)

  const handlePuan = (kriterAdi, kriterPuan) => {
    puanKaydet(id, { [kriterAdi]: kriterPuan })
  }

  const toggleKategori = (katId) => setAcikKategoriler(p => ({ ...p, [katId]: !p[katId] }))

  const kategoriIstatistik = (katId) => {
    const katKriterler = aktifKriterler.filter(k => k.kategoriId === katId)
    const puanlanan = katKriterler.filter(k => ev.puanlar?.[k.id] !== undefined)
    const katYuzde = puanlanan.length > 0
      ? toplamPuanHesapla(katKriterler, ev.puanlar || {}).yuzde
      : null
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        <button onClick={() => navigate('/')} className="p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">
          <ArrowLeft size={20} />
        </button>
      ...
      <div className="card mb-5">
        <div className="flex items-center gap-5">
          <SkorDairesi kriterler={aktifKriterler} puanlar={ev.puanlar || {}} boyut={110} />
          <div className="flex-1">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Değerlendirilen</span>
                <span className="font-semibold text-gray-900">{tamamlananKriter}/{aktifKriterler.length}</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-blue-500 rounded-full transition-all"
                  style={{ width: `${aktifKriterler.length > 0 ? (tamamlananKriter / aktifKriterler.length) * 100 : 0}%` }}
                />
              </div>
              {ev.fiyat && (
              ...
              {ev.not && (
                <p className="text-xs text-gray-500 italic line-clamp-2">"{ev.not}"</p>
              )}
            </div>
        ...
        {gosterDetay && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <p className="text-xs font-semibold text-gray-600 mb-3 uppercase tracking-wide">Kategori Bazlı Özet</p>
            <div className="space-y-2">
          ...
          return (
            <div key={kat.id} className="border border-gray-200 rounded-xl overflow-hidden">
              ...
              >
                ...
                {acikKategoriler[kat.id] ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
              </button>
                    ...
                    <div key={kriter.id} className={i > 0 ? 'pt-4' : ''}>
                      <div className="flex items-center gap-1 mb-2">
                        <div className="flex gap-0.5">
                          {[1,2,3,4,5].map(n => (
                            <div key={n} className={`h-1 w-3 rounded-full ${n <= kriter.agirlik ? 'bg-blue-400' : 'bg-gray-200'}`} />
                          ))}
                        </div>
                      ...
                      <PuanSecici
                        ...
                        mevcutPuan={ev.puanlar?.[kriter.id]}
